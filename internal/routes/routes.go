...
func Register(app *fiber.App, db *pgxpool.Pool, cfg config.Config) *services.AuthService {
	...
	teacherInviteRepo := repositories.NewTeacherInviteRepository(db)
	...
	teacherInviteService := services.NewTeacherInviteService(teacherInviteRepo)
	...
	invitationService := services.NewInvitationService(studentService, teacherInviteService, eventSettingsService)
	attendanceService := services.NewAttendanceService(studentRepo, teacherInviteRepo)
	...
	teacherInviteHandler := handlers.NewTeacherInviteHandler(teacherInviteService)
	...
	schoolAdmin.Get("/teacher-invites", teacherInviteHandler.List)
	schoolAdmin.Post("/teacher-invites", teacherInviteHandler.Create)
